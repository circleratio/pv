...
- 依存: Step 31, Step 32
- 全モジュールを結合したアプリケーションに対して、spec.md 5章「テスト設計」の36項目（起動、ページ送り、境界、終了、レーザーポインター、リサイズ、フィット表示、ファイル履歴、空白ウィンドウからのオープン、PDFオープン時のウィンドウリサイズ、ドラッグ＆ドロップ、全画面モード、右クリックメニューの画面端クランプ）を手動シナリオとして実施する。
- 併せて `npm test` / `cargo test` を通しで実行し、これまでの自動テストがすべてgreenであることを確認する。
- **完了条件**: 自動テストが全てgreen、かつspec.md 5章の36項目すべてが合格する。
