...
| Esc | 終了（スライド一覧表示中はまず一覧表示を閉じる。全画面モード中は先に全画面を解除） |
| 左クリック押しっぱなし＋ドラッグ | レーザーポインター（オレンジの線で軌跡を強調表示） |
| 右クリック | 「ファイルを開く」（ファイル選択ダイアログ）・全画面モードのトグル・スライド一覧表示のトグル・ファイル履歴（最大10件、ローカルに保存）のメニューを表示 |
...
右クリックメニューから「スライド一覧表示」に切り替えると、全スライドの番号とサムネイルをグリッド表示できます（PDF未読込時は選択できません）。スライドをダブルクリックするとそのスライドの通常表示に戻ります。
